 ...
 * CSS gradients for the dynamic background.
 */
const AnimatedGradientBackground: React.FC<AnimatedGradientBackgroundProps> = ({
  ...
  useEffect(() => {
    ...
    const buildGradient = (w: number) => {
      const gradientStopsString = gradientStops
        .map((stop, index) => `${gradientColors[index]} ${stop}%`)
        .join(", ");
      return `radial-gradient(${w}% ${w + topOffset}% at 50% 20%, ${gradientStopsString})`;
    };

    // Static case: paint once, no animation loop.
    if (!Breathing) {
      if (containerRef.current) {
        containerRef.current.style.background = buildGradient(width);
      }
      return;
    }
